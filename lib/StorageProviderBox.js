...
StorageProviderBox.prototype.setup = function(fn) {
  ...
  if(!raidFolderID) {
    this.getOrCreateRaidFolder(raidFolderName, function(folderId) {
      if(folderId) {
        console.log("Got Raid Folder");
        raidFolderID = folderId;
      } else {
        console.log("Unable to get raid folder");
      }
      fn(null);
    });
  } else {
    ...
    fs.writeFile(info.path, data, function(err) {
      ...
      .end(function (response) {
         switch(response.status) {
            case 201:
                console.log("File uploaded successfully. ID = " + response.body.entries[0].id);
                console.log("SHA1 checksum: " + response.body.entries[0].sha1);
                break;
            case 409:
  ...
  this.findFileByName(raidFolderID, file, function(fileID) {
    if(fileID) {
      ...
      .end(function (response) {
        ...
        fs.writeFile("/Users/christophertunkl/Desktop/body.png", response.body, 'binary');
      });
    } else {
    ...
    if(fileID) {
      ...
      .end(function (response) {
        if(response.status === 204) {
          ...
          console.log("Unable to delete file. Error "+response.status);
        }
        fn();
      });
    } else {
...
StorageProviderBox.prototype.getRootFolders = function(fn) {
  this.getFolderContent(0, function(items) {
    if(items) {
      ...
      console.log("Unable to read root folders");
      fn(null);
    }
  })
}
...
StorageProviderBox.prototype.getFolderContent = function(folderId, fn) {
  ...
  .end(function (response) {
    if(response.status === 200) {
      ...
      console.log("Unable to read content of folder " + folderId + ". Error "+response.status);
      fn(null);
    }
  });
}
