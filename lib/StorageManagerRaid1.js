            ...
            self.calculateHash(data, function (err, hash) {
                ...
                self.createFilenameWithVersion(filename, version, function (filenameWithVersion) {
                    self.forEachProvider(function (provider, callback) {
                        provider.createFile(self.username+"/"+self.type, filenameWithVersion, data, callback);
                    }, function (err) {
                        if (err)
                            return fn(err);
            ...
            self.forEachProvider(function (provider, callback) {
                ...
                if (correctData) {
                    async.forEach(incorrectProviders, function (incorrectProvider, callback) {
                        ...
                        self.createFilenameWithVersion(filename, version, function (filenameWithVersion) {
                            incorrectProvider.deleteFile(self.username+"/"+self.type, filenameWithVersion, function (err) {
                                //TODO: Is deleting of an non-existing file an error too?
                                if (err)
                                    return callback(err);

                                incorrectProvider.createFile(self.username+"/"+self.type, filenameWithVersion, correctData, callback);
                            });
                        });
                    }, function (err) {
