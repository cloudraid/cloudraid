  ...
  self.collection(function(err, collection) {
    ...
    self.calculateHash(data, function(err, hash) {
      ...
      self.forEachProvider(function(provider, callback) {
        provider.createFile(filename, data, callback);
      }, function(err) {
        if (err)
          return fn(err);
...
StorageManagerRaid1.prototype.readFile = function(filename, fn) {
  ...
  self.collection(function(err, collection) {
    ...
    collection.findOne({
      ...
      self.forEachProvider(function(provider, callback) {
        provider.readFile(filename, function(err, data) {
          if (err)
            return callback(err);
        ...
        if (correctData) {
          async.forEach(incorrectProviders, function(incorrectProvider, callback) {
            ...
            incorrectProvider.deleteFile(filename, function(err) {
              //TODO: Is deleting of an non-existing file an error too?
              if (err)
                return callback(err);

              incorrectProvider.createFile(filename, correctData, callback);
            });
          }, function(err) {
...
StorageManagerRaid1.prototype.deleteFile = function(filename, fn) {
  ...
  self.collection(function(err, collection) {
    ...
    self.forEachProvider(function(provider, callback) {
      provider.deleteFile(filename, callback);
    }, function(err) {
      if (err)
        return fn(err);
