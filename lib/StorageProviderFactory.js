...
module.exports.create = function(credentials, settingsCallback, fn) {
    try {

        if (credentials.type === 'amazon')
            return fn(null, new StorageProviderAmazon(credentials, settingsCallback));
        if (credentials.type === 'box')
            return fn(null, new StorageProviderBox(credentials, settingsCallback));
        if (credentials.type === 'dropbox')
            return fn(null, new StorageProviderDropbox(credentials, settingsCallback));
...
module.exports.getAvailableStorageProviders = function(fn) {
    if(!availableStorageProviders) {
        availableStorageProviders = new Array();
