...
StorageProviderDropbox.prototype.createFile = function(path, file, data, fn) {
    var self = this;
    self.client.writeFile(path+"/"+file, data, function(error, stat) {
        if (error) {
            self.log(error);
        }
        else{
...
StorageProviderDropbox.prototype.readFile = function(path, file, fn) {
    var self = this;
    self.client.readFile(path+"/"+file, {buffer : true }, function(error, data) {
        if (error) {
            self.log(error);
            fn(null);
        }
...
StorageProviderDropbox.prototype.deleteFile = function(path, file, fn) {
    var self = this;
    self.client.remove(path+"/"+file, function(error, stat){
    if (error) {
        self.log(error);
    }
    else{
