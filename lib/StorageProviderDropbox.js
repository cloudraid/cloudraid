...
StorageProviderDropbox.prototype.createFile = function(file, data, fn) {
    var self = this;
    self.client.writeFile(file, data, function(error, stat) {
        if (error) {
            self.log(error);
        }
        else{
...
StorageProviderDropbox.prototype.readFile = function(file, fn) {
    var self = this;
    self.client.readFile(file, {buffer : true }, function(error, data) {
        if (error) {
            self.log(error);
            fn(error, null);
        }
...
StorageProviderDropbox.prototype.deleteFile = function(file, fn) {
    var self = this;
    self.client.remove(file, function(error, stat){
    if (error) {
        self.log(error);
    }
    else{
